    ...
    if connection.provider == CalendarProvider.GOOGLE:
        return _request(
            ...
            json={
                ...
                "conferenceData": {
                    "createRequest": {
                        "requestId": f"klaria-{int(utc_now().timestamp())}",
    ...
    for raw_item in raw:
        ...
        if not event:
            event = CalendarEvent(
                ...
                auto_join=connection.auto_join_tagged and "[klaria]" in item["title"].lower(),
            )
        else:
            ...
            if previous_attendees != event.attendees_json and event.consent_session_id:
                ...
                event.invitation_errors_json = json.dumps(
                    ["Invités modifiés : vérifiez et réactivez Klaria"]
                )
        event.status = (
