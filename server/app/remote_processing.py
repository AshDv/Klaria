...
def _stop_requested(messages: list[dict]) -> dict | None:
    return next(
        (
            ...
            and not str(item.get("text") or "").startswith("Nolya est présent")
            and "nolya" not in _plain(chat_sender(item))
...
def _welcome_message() -> str:
    return (
        "Nolya est présent et transcrit cette réunion avec l'accord des participants. "
        "Écrivez « STOP NOLYA » dans ce chat pour demander l'arrêt immédiat."
    )
...
def _recap_message(meeting: RemoteMeeting, report) -> str:
    ...
    message = f"Compte rendu Nolya prêt\n\n{report.executive_summary}"
    if action_text:
        message += f"\n\nActions principales :\n{action_text}"
...
def finalize_remote_meeting(meeting_id: str) -> None:
    ...
    with Session(engine) as db:
        ...
        if not segments or not meeting.transcript:
            meeting.status = RemoteMeetingStatus.FAILED
            meeting.error = (
                "Aucune parole n'a été transcrite. Vérifiez que l'hôte a admis Nolya "
