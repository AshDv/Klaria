...
function MeetingSetup({ onCreated }) {
  ...
  return <section className="page">
    ...
    <form className="content-card meeting-form" onSubmit={create}>
      ...
      <p className="privacy-hint">Ajoutez toutes les personnes dont la voix peut être captée, vous compris. Nolya utilise ces adresses uniquement pour envoyer et prouver le consentement.</p>
...
function ConsentStatus({ meeting, onChange }) {
  ...
  async function start() {
    if (!notice) {
      setError("Annoncez la présence de Nolya aux personnes dans la salle.");
      return;
    }
    try {
  ...
  return <section className="page">
    ...
    <div className="content-card consent-dashboard">
      ...
      <label className="consent"><input type="checkbox" checked={notice} onChange={(event) => setNotice(event.target.checked)} /><span><strong>J’annonce Nolya à toutes les personnes présentes</strong><small>Je leur rappelle que l’enregistrement peut être arrêté immédiatement.</small></span></label>
      {error && <div className="alert error">{error}</div>}
