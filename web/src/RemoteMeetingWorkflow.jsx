...
export function NewMeeting({ onRemoteCreated, onRecordingCreated }) {
  ...
  return <section className="page">
    ...
    <div className="mode-switch" role="tablist">
      <button className={mode === "bot" ? "active" : ""} onClick={() => setMode("bot")}><span className="mode-icon">S</span><strong>Meet ou Teams</strong><small>Nolya rejoint la réunion</small></button>
...
function BotSetup({ onCreated }) {
  ...
  return <div className="launch-grid">
    <form className="content-card bot-form" onSubmit={create}>
      <div className="form-intro"><span className="platform-pair"><i><PlatformIcon platform="google_meet" decorative/></i><i><PlatformIcon platform="teams" decorative/></i></span><div><h2>Inviter l’assistant</h2><p>Nolya apparaîtra comme un participant visible.</p></div></div>
      ...
      {manual && <>
      <label className="calendar-drop"><input type="file" accept=".ics,text/calendar" onChange={importInvitation}/><strong>Importer une invitation calendrier</strong><span>Nolya lit automatiquement le lien, l’heure et les invités du fichier .ics.</span></label>
      ...
      <p className="privacy-hint">Chaque personne reçoit un lien individuel. Le bot reste bloqué jusqu’au dernier accord. Nolya ne capture ni la vidéo ni le partage d’écran.</p>
    ...
    <aside className="launch-aside"><span className="card-label">Déroulé</span><Step number="01" title="Accords" text="Chacun confirme par e-mail."/><Step number="02" title="Admission" text="L’hôte accepte Nolya dans la réunion."/><Step number="03" title="Compte rendu" text="Le direct devient décisions et actions."/></aside>
  </div>;
}
...
function BotConsent({ setup, onCreated }) {
  ...
  async function inviteBot() {
    if (!notice) { setError("Confirmez que tous les participants voient que Nolya va rejoindre."); return; }
  ...
  return <div className="consent-stage">
    <div className="content-card consent-dashboard bot-consent-card">
      ...
      {error && <div className="alert error">{error}</div>}
      <button className="primary-button compact" onClick={inviteBot} disabled={!meeting.all_consented || busy}>{busy ? "Connexion de Nolya…" : "Inviter Nolya maintenant"}</button>
    </div>
  </div>;
}
