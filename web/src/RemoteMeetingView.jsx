...
export function RemoteMeetingView({ id, onBack }) {
  ...
  async function stopNow() {
    if (!confirm("Arrêter Klaria et effacer définitivement toutes les données de ce direct ?")) return;
...
function Joining({ meeting }) { return <div className="joining-layout"><article className="joining-card"><div className="bot-avatar"><span>K</span><i/></div><span className="eyebrow">Klaria demande à entrer</span><h2>Admettez le bot dans {meeting.platform === "google_meet" ? "Google Meet" : "Microsoft Teams"}</h2><p>Il apparaîtra sous le nom « Klaria — prise de notes ». La transcription commencera dès son admission.</p><div className="waiting-line"><span className="spinner"/> En attente de l’hôte</div></article><aside className="content-card admission-check"><span className="card-label">À vérifier</span><ol><li>La réunion est déjà ouverte.</li><li>L’hôte voit la demande de Klaria.</li><li>Tous les participants savent que le bot transcrit.</li></ol></aside></div>; }

function LiveRoom({ meeting }) {
  const speakers = new Set(meeting.segments.map((item) => item.speaker)).size;
  return <div className="live-layout"><main className="live-transcript"><div className="live-heading"><div><span className="live-pulse"/>Transcription en direct</div><span>{meeting.segments.length} passages</span></div><div className="transcript-stream">{meeting.segments.length ? meeting.segments.map((segment) => <div className="speech-row" key={segment.id}><span className="speaker-avatar">{segment.speaker.slice(0, 1).toUpperCase()}</span><div><strong>{segment.speaker}<small>{formatClock(segment.start)}</small>{segment.speaker_mapping_status === "NO_SPEAKER_EVENTS" && <em>identité incertaine</em>}</strong><p>{segment.text}</p></div></div>) : <div className="listening-empty"><span className="sound-ring"/><h3>Klaria écoute</h3><p>Les premières phrases vont apparaître ici.</p></div>}</div></main><aside className="live-rail"><article><span className="card-label">Session</span><strong>{formatDuration(meeting.duration_seconds)}</strong><small>temps capté</small></article><article><span className="card-label">Voix</span><strong>{speakers || "—"}</strong><small>intervenants détectés</small></article><article className="chat-control"><span className="card-label">Contrôle dans le chat</span><strong>STOP KLARIA</strong><small>arrête immédiatement la capture. Si l’auteur est identifié, ses passages et les passages incertains sont supprimés ; sinon tout est effacé.</small></article><article className="privacy-live"><span>✓</span><div><strong>{meeting.media_recording_enabled ? "Replay audio autorisé" : "Audio non conservé"}</strong><small>{meeting.media_recording_enabled ? `Suppression après ${meeting.media_retention_days} jour${meeting.media_retention_days > 1 ? "s" : ""}.` : "Seul le texte utile est gardé."}</small></div></article></aside></div>;
}

function Finalizing({ meeting }) { return <div className="analysis-stage"><div className="analysis-orbit"><span>K</span><i/><i/><i/></div><span className="eyebrow">Analyse en cours</span><h2>Klaria relie chaque décision à ses preuves</h2><p>{meeting.segments.length} passages sont organisés en résumé, décisions, actions, risques et brief audio.</p><div className="skill-pills">{meeting.skills.map((skill) => <span key={skill.key}>✓ {skill.title}</span>)}</div></div>; }
